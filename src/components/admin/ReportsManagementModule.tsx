...
import { downloadTextFile, buildCsv } from '../../utils/fileExport';
...
export const ReportsManagementModule: React.FC<ReportsManagementModuleProps> = ({
  ...
  const handleExportFile = (rep: ReportRecord, fmt: 'PDF' | 'Excel' | 'CSV') => {
    const csvData = buildCsv(
      ['Report ID', 'Title', 'Category', 'Generated Date', 'Generated By', 'Total Records', 'Summary'],
      [[rep.id, rep.title, rep.category, rep.generatedDate, rep.generatedBy, rep.totalRecords, rep.summaryText]]
    );
    const extension = fmt === 'PDF' ? 'pdf' : fmt === 'Excel' ? 'xlsx' : 'csv';
    downloadTextFile(
      csvData,
      `${rep.title.replace(/\s+/g, '_')}_${fmt.toLowerCase()}.${extension}`,
